...
export const creaHTMLlistaLlibres = (llistallibres, llistaautors,llistaeditorials,llistageneres) =>{
    ...
    <thead><tr><th>#Id</th><th>Llibres</th><th>Autors</th><th>Gènere</th><th>Editorials</th><th>Valoració</th><th class='esborrar invisible'>Esborrar</th></tr></thead><tbody>`
    ...
    for (let i of llistallibres.llibres)  {
        ...
        html += `<tr ><td>${i.id_llibre}</td><td>${i.titol}</td><td>${cad_autors}</td><td>${cad_generes}</td><td>${cad_editorials}</td><td>${i.valoracio}</td>
